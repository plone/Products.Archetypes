...
class HoldingReference(Reference):
    def beforeTargetDeleteInformSource(self):
        raise ReferenceException("Can't delete target, its held by %s" % self.getSourceObject().absolute_url())
...
class FolderishReference(Reference):
    ...
    def __init__(self, id, sid, tid, relationship=FOLDERISH_REFERENCE, **kwargs):
        Reference.__init__(self, id, sid, tid, relationship, **kwargs)
        
    def beforeSourceDeleteInformTarget(self):
        # The idea is that a reference folder would be incrementing the reference count on its children
        # but not actually, placefully contain the object.
        # if the count is not 1 (meaning the last reference) then we reject the delete
